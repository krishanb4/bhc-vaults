...
const mS_PROPNM_DESCRIP = 'description',
    mS_PROPNM_NM = 'name',
    mS_PROPNM_SYMBL = 'symbol',
    mS_PROPNM_ADDR = 'address',
    mS_PROPNM_SITE = 'website';
const mo_SRC = {};
...
async function p_loadChain(O_CHN) {
    ...
    mo_SRC[O_CHN.S_DIR] = (
        await
        import (
            `../../node_modules/blockchain-addressbook/build/address-book/${O_CHN.S_DIR}/tokens/tokens.js`
        )
    )['tokens'];
...
async function p_main() {
    ...
    for (const O_CHN of mAO_CHAIN) {
        ...
        for (const S_TKN in mo_SRC[O_CHN.S_DIR]) {
            ...
            if (O[mS_PROPNM_DESCRIP] && O[mS_PROPNM_DESCRIP].trim()) continue;
    ...
    FS.writeFileSync(
        `${S_DIR_BASE}/beefy-v2/tokensWithoutDescrip.txt`,
        JSON.stringify(moao_trgt, null, 2)
    );
