...
async function p_loadChain(O_CHN) {
    ...
    //  the way
    mo_trgt[O_CHN.S_FILENM] = (await
        import (`../config/vault/${O_CHN.S_FILENM}.js`)).pools.reduce(
        (o_vlts, O_VLT) => {
            const O = o_vlts[O_VLT[mS_PRPNM_CTRCT]];
            if (O) {
                if (!Array.isArray(O)) o_vlts[O_VLT[mS_PRPNM_CTRCT]] = [O];
                o_vlts[O_VLT[mS_PRPNM_CTRCT]].push(O_VLT);
            } else o_vlts[O_VLT[mS_PRPNM_CTRCT]] = O_VLT;
            return o_vlts;
        }, {}
    );
...
async function p_loadAllVaultData() {
    const Af = [];
    for (const O_CHN of mAO_CHAIN) Af.push((async() => await p_loadChain(O_CHN))());
...
async function p_main() {
    ...
    for (const O_CHN of mAO_CHAIN) {
        ...
        for (const S_CTRCT in o_trgtChn) {
            ...
            if (-1 == I_EOL) {
                ...
                const O_note = {
                    [S_PRPNM_CTRCT]: `${O_CHN.S_FILENM}: ${S_CTRCT}`,
                    [S_PRPNM_ERR_DUPES]: [],
                };
                ...
                O_ctrct.forEach((O, I) => {
                    ...
                    if (I < O_ctrct.length - 1) o_trgtChn[`${S_CTRCT}-${I}`] = O;
                    else o_trgtChn[S_CTRCT] = O;
                });
            ...
            O_ctrct.forEach((O_VLT, I) => {
                ...
                if (!O_REGX_EOL.test(s)) {
                    ...
                    else if (!o_note[S_PRPNM_REMVD]) o_note[S_PRPNM_REMVD] = s;
                    else if (!o_note[S_PRPNM_REMVD].isArray())
                        o_note[S_PRPNM_REMVD] = [o_note[S_PRPNM_REMVD], s];
                    else o_note[S_PRPNM_REMVD].push(s);
                    return;
                } //if (!(O_REGX_EOL.test( s)))
                ...
                if (!(o_note || o_note[S_PRPNM_ERR_DUPES])) {
                    ...
                    else o_note[S_PRPNM_ERR_DUPES] = [S];
                    console.error(
                        `${O_CHN.S_FILENM}: Duplicate EOL'd vaults with address ${S_CTRCT}\n  - ${S}`
                    );
                } //if (!(o_note ||
                console.error(`  - ${s}`);
                o_note[S_PRPNM_ERR_DUPES].push(s);
            ...
            if (o_note) aO_noted.push(o_note);
            ...
            if (o_note[S_PRPNM_REMVD]) o_trgtChn.b_dirty = true;
        } //for (const S_CTRCT in o_trgtChn)
    } //for (const O_CHN of mAO_CHAIN)
    ...
    //  overlord attention
    FS.writeFileSync(
        'prunedEol&errorVaults.txt',
        JSON.stringify(aO_noted, null, 2).replace(/"([^"]+)":/g, '$1:')
    );
    ...
    for (const O_CHN of mAO_CHAIN) {
        ...
        FS.writeFileSync(
            `src/config/vault/${O_CHN.S_FILENM}.js`,
            `export const pools = ${JSON.stringify(Object.values(o_trgtChn), null, 2).replace(
        /"([^"]+)":/g,
        '$1:'
      )};`
        );
