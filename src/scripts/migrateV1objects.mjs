...
const mAO_CHAIN = [
    ...
    mS_PRPNM_ID = 'id',
    mS_PRPNM_ASSTS = 'assets',
    mS_PRPNM_STRAT_TYP = 'stratType',
    mS_PRPNM_CHAIN = 'network',
    mS_PRPNM_LOGO = 'logo',
    mS_PRPNM_RISKS = 'risks',
    mS_PRPNM_CTRCT = 'earnContractAddress',
    mS_PRPNM_TYP = 'type',
    mS_PRPNM_DEPOST_PSD = 'depositsPaused',
    mS_PRPNM_STATUS = 'status',
    mS_PRPNM_ID_VLT = 'poolId',
    mS_PRPNM_PTNRS = 'partners',
    mS_PRPNM_SOCL = 'social',
    mS_PRPNM_TKN = 'tokenAddress',
    mS_PRPNM_VLT_ID = 'poolId',
    mS_PRPNM_PTNR_CTX = 'O_CTX',
    mS_PRPNM_MOD = 'O_MOD',
    mS_PRPNM_CMN_PTNRS = 'AO_PTNRS',
    mS_PRPNM_PTNR = 'O_ptnr',
    mS_PRPNM_NM = 'S_NM',
    mS_PRPNM_CHKD = 's_chkd',
    mS_DIR_BASE =
    import.meta.url.slice('file://'.length, 1000).split('/').slice(0, -4).join('/'),
...
async function P_loadChainVaults(O_CHN) {
    ...
    [moAO_SRC_VLTS[O_CHN.S_SRC], mo_trgtVlts[O_CHN.S_SRC]] = await Promise.all([
        (async() =>
            (
                await
                import (`../../../beefy-app/src/features/configure/vault/${O_CHN.S_SRC}_pools.js`)
            )[O_CHN.S_SRC + 'Pools'])(),
        (async() =>
            await
            import (
                `../config/vault/${O_CHN.S_TRGT_ALIAS ? O_CHN.S_TRGT_ALIAS : O_CHN.S_SRC}.tsx`
            ).then(
                O_MOD =>
                O_MOD.pools.reduce((o, O) => {
                    if (o[O[mS_PRPNM_ID]])
...
function P_loadAllVaultData() {
    const aP = [];
    for (const O_CHN of mAO_CHAIN) aP.push(P_loadChainVaults(O_CHN));
...
async function Po_resolveVaults() {
    ...
    for (const O_CHN of mAO_CHAIN) {
        const S_TYP_RMVD = 'REMOVED',
            S_TYP_ADD = 'Added',
            S_TYP_UPDT = 'synced',
            S_PRPNM_LGO = 'logo-copied',
            S_PRPNM_EOLD = 'EOLd',
        ...
        for (const O_SRC of moAO_SRC_VLTS[O_CHN.S_SRC]) {
            ...
            if (
                (o_trgt =
                    o_trgtChn[S_ID_SRC] ||
                    ((s = S_ID_SRC.split(/[-=]eol[0-9a-z]?$/)[0]) &&
                        S_ID_SRC !== s &&
                        (o = o_trgtChn[s]) &&
                        O_SRC[mS_PRPNM_CTRCT] === o[mS_PRPNM_CTRCT] ?
                        o :
                        null))
            ) {
                ...
                for (const S in O_SRC) {
                    ...
                    if (mS_PRPNM_ASSTS === S || mS_PRPNM_RISKS === S) {
                        ...
                        if (
                            o_trgt[S] &&
                            O_SRC[S].length == o_trgt[S].length &&
                            o_trgt[S].every((s, i) => O_SRC[S][i] === s)
                        )
                            continue;
                        ...
                        if (b_pausedStatusProcessd) continue;
                        ...
                        if (b_pausedPrp) {
                            ...
                            (b_eol = 'eol' === O_SRC[S]) ||
                            (O_SRC[mS_PRPNM_DEPOST_PSD] && 'paused' !== o_trgt[mS_PRPNM_STATUS])
                        ) {
                            if (b_eol) o_trgt[S] = O_SRC[S];
                            else o_trgt[mS_PRPNM_STATUS] = O_SRC[mS_PRPNM_DEPOST_PSD] ? 'paused' : 'active';
                            O_hit[mS_PRPNM_TYP] = S_TYP_UPDT;
                        } //if (b_pausedPrp)
                        ...
                        if (mS_PRPNM_LOGO === S)
                            try {
                                ...
                                console.error(
                                    `  Failed to copy over obstensibly updated logo of vault ${O_SRC[mS_PRPNM_ID]}:\n   beefy-app/src/images/${O_SRC[S]} (Error: ${O}`
                                );
                            }
                    } //if (mS_PRPNM_ASSTS === S)
                } //for (const S in O_SRC)
                ...
                if (O_hit[mS_PRPNM_TYP]) o_trgtChn.b_dirty = true;
            ...
            if (!S_LOGO) {
                ...
                if (!o_DirSingleLogo) {
                    ...
                    try {
                        ...
                        o_DirSingleLogo = false;
                        o_Dirent = null; //sentinels to shut down any work
                    ...
                    for (s_ASST of O_SRC[mS_PRPNM_ASSTS]) {
                        ...
                        if (!s && o_Dirent) {
                            while ((o_Dirent = o_DirSingleLogo.readSync())) {
                                ...
                                if (!(
                                        o_Dirent.isFile() &&
                                        (i = o_Dirent.name.lastIndexOf('.')) > 0 &&
                                        ++i < o_Dirent.name.length - 1 && ['svg', 'webp', 'png'].includes(
                                            o_Dirent.name.slice(i - o_Dirent.name.length).toLowerCase()
                                        )
                                    ))
                                    continue;
                        ...
                        } else if (S_PROCESSD === s) continue;
                        if (s) {
                            o_singleLogo[s_ASST] = S_PROCESSD;
                            mO_FS.copyFileSync(
                                `${mS_DIR_BASE}/beefy-app/src/images/single-assets/${s}`,
                                `${mS_DIR_BASE}/beefy-v2/src/images/single-assets/${s}`
                            );
                        } else
                            console.error(
                                `  Failed to locate icon for ${O_SRC[mS_PRPNM_CHAIN]} asset ${s_ASST} of new vault ${O_SRC[mS_PRPNM_ID]}`
                            );
                    } //for (s_ASST of O_SRC[ mS_PRPNM_ASSTS])
                } catch (O) {
                    console.error(
                        `  Failed to copy over an expected icon for ${O_SRC[mS_PRPNM_CHAIN]} asset ${s_ASST} of new vault ${O_SRC[mS_PRPNM_ID]}\n   (Error: ${O}`
                    );
                } //try
                ...
                try {
                    ...
                    console.error(
                        `  Failed to copy over specified logo of new vault ${O_SRC[mS_PRPNM_ID]}:\n   beefy-app/src/images/${S_LOGO} (Error: ${O}`
                    );
                }
            ...
            for (const S in O_PROP_IGNR)
                if (S in o) delete o[S];
            if (O_SRC[mS_PRPNM_RISKS]) o[mS_PRPNM_RISKS] = [...O_SRC[mS_PRPNM_RISKS]];
            if (o[mS_PRPNM_ASSTS]) {
                ...
                console.error(
                    `  Asset list missing on source vault ${o[mS_PRPNM_ID]} on ${o[mS_PRPNM_CHAIN]} chain`
                );
            ...
            if (O_SRC[mS_PRPNM_DEPOST_PSD]) delete o[mS_PRPNM_DEPOST_PSD];
        ...
        Object.values(o_trgtChn).forEach(O_trgt => {
            if (!(O_trgt && Object === O_trgt.constructor) ||
                (s = O_trgt[mS_PRPNM_ID]).includes('bifi-gov') ||
                'beefy-beFTM-earnings' === s ||
                'beefy-beJoe-earnings' === s ||
                s.endsWith('-earnings')
            )
                return;
            ...
            O_hits[S] = {
                [mS_PRPNM_ID]: S,
                [mS_PRPNM_TYP]: S_TYP_RMVD
            };
            i_pruned++;
        }); //Object.values( o_trgtChn).forEach(
        if (i_pruned > I) o_trgtChn.b_dirty = true;
        ...
        //	been located
        if (!b_dirty) b_dirty = o_trgtChn.b_dirty;
    } //for (const O_CHN of mAO_CHAIN)
...
function persistVaultsSync({ I_ADDED, I_PRUNED }) {
    ...
    for (const O_CHN of mAO_CHAIN) {
        ...
        if (!o_VLTS.b_dirty) continue;
        delete o_VLTS.b_dirty;
        console.log('Writing updated v2 vault descriptors file: ' + O_CHN.S_SRC);
        mO_FS.writeFileSync(
            `${mS_DIR_BASE}/beefy-v2/src/config/vault/${
        O_CHN.S_TRGT_ALIAS ? O_CHN.S_TRGT_ALIAS : O_CHN.S_SRC
      }.tsx`,
            `export const pools = ${JSON.stringify(Object.values(o_VLTS), null, 2).replace(
        /"([^"]+)":/g,
        '$1:'
      )};`
        );
    ...
    for (const O_CHN of mAO_CHAIN) {
        ...
        AO_PTNRS.forEach(O_PTNR => {
            ...
            if (!O_PTNR[mS_PRPNM_CHKD]) {
                console.log(`  Removing now-unused common-partner const '${O_PTNR[mS_PRPNM_NM]}'`);
                return;
            ...
            //	note of it
            aS.push(
                `const ${O_PTNR[mS_PRPNM_NM]} = ` + JSON.stringify(O_PTNR[mS_PRPNM_PTNR], null, 2) + ';'
            );
        }); //AO_PTNRS.forEach( O_PTNR =>
        ...
        const S_BSTS =
            'export const pools = ' + (aS.length ? TrgtCommonPartner.s_deJSONtag(s) : s) + ';';

        mO_FS.writeFileSync(
            `${mS_DIR_BASE}/beefy-v2/src/config/boost/${
        O_CHN.S_TRGT_ALIAS ? O_CHN.S_TRGT_ALIAS : O_CHN.S_SRC
      }.tsx`,
            ((aS.length ? aS.join('\n\n') + '\n\n\n' : '') + S_BSTS).replace(/"([^"]+)":/g, '$1:')
        );
...
async function P_loadSrcBoosts(S_CHN) {
    ...
    let s_src = mO_FS
        .readFileSync(`${mS_DIR_BASE}/beefy-app/src/features/configure/stake/${S_CHN}_stake.js`)
        .toString()
        .replace(/^.* govPoolABI[ ,].*$/gm, '');
    let aS = [];
    s_src = s_src.replace(
        /^const ([^\s]+) =/gm,
        (S_MTCH, S_VAR) => (aS.push(S_VAR), 'export ' + S_MTCH)
    );
    ...
    const O_MOD = await
    import (
        'data:text/javascript;base64,' + Buffer.from(s_src).toString('base64')
    ),
    aO_src = O_MOD[S_CHN + 'StakePools'];
    if (aS.length) {
        let aO = [];
        aS.forEach(S => aO.push({
            [mS_PRPNM_PTNR]: O_MOD[S],
            [mS_PRPNM_NM]: S
        }));
        aO_src[Symbol.for(mS_PRPNM_PTNR_CTX)] = {
            [mS_PRPNM_MOD]: O_MOD,
            [mS_PRPNM_CMN_PTNRS]: aO
        };
    }
    return aO_src;
} //P_loadSrcBoosts(
...
class TrgtCommonPartner {
    constructor(O_SRC, S_CHKD) {
        Object.assign(this, O_SRC);
        if (S_CHKD) this[mS_PRPNM_CHKD] = S_CHKD;
    }
...
async function P_loadTrgtBoosts(O_CHN) {
    ...
    s_src = s_src.replace(
        /^const ([^\s]+) =/gm,
        (S_MTCH, S_VAR) => (aS_VAR.push(S_VAR), 'export ' + S_MTCH)
    );
    ...
    import (
        ...
        (o_MOD = O_MOD).pools.reduce((o, O) => {
            if (o[O[mS_PRPNM_ID]]) throw `Duplicate ${O_CHN.S_SRC} v2 boost-ID (${O[mS_PRPNM_ID]}`;
            o[O[mS_PRPNM_ID]] = O;
            return o;
        }, {}),
        () => ({})
    ...
    if (aS_VAR.length)
        aS_VAR.forEach(S =>
            aO.push(new TrgtCommonPartner({
                [mS_PRPNM_PTNR]: o_MOD[S],
                [mS_PRPNM_NM]: S
            }))
        );
    O_trgt[Symbol.for(mS_PRPNM_PTNR_CTX)] = {
        [mS_PRPNM_MOD]: o_MOD,
        [mS_PRPNM_CMN_PTNRS]: aO
    };
    return O_trgt;
...
function P_loadAllBoostData(oAO_SRC_BSTS, o_trgtBsts) {
    const aP = [];
    for (const O_CHN of mAO_CHAIN) aP.push(P_loadChainBoosts(O_CHN, oAO_SRC_BSTS, o_trgtBsts));
...
function copyOverSrcImage(S) {
    ...
    if (S_PTH && !mO_FS.existsSync(S_PTH)) mO_FS.mkdirSync(S_PTH, { recursive: true });
    mO_FS.copyFileSync(
        `${mS_DIR_BASE}/beefy-app/src/images/${S}`,
        `${mS_DIR_BASE}/beefy-v2/src/images/${S}`
    );
} //copyOverSrcImage(
...
//	Consider logging these as actions to be taken into the output object so that the side
//	effect can be removed, if perhaps only for this function.
async function Po_resolveBoosts(OAO_SRC_VLTS, oAO_SRC_BSTS, o_trgtBsts) {
    ...
    for (const O_CHN of mAO_CHAIN) {
        ...
        const O_SRC_VLTS = OAO_SRC_VLTS[O_CHN.S_SRC].reduce((o_vlts, O_VLT) => {
            const S = O_VLT[mS_PRPNM_CTRCT],
                S_LWR = S.toLowerCase();
            if (o_vlts[S_LWR])
                console.error(
                    `  Duplicate source-vault contract uncovered:\n    id: ${O_VLT[mS_PRPNM_ID]}\n    contract: ${S}`
                );
            else o_vlts[S_LWR] = O_VLT;
            return o_vlts;
        }, {});

        //for each source boost on the chain...
        const S_NTV = (await
            import ('blockchain-addressbook')).addressBook[
            O_CHN.S_ABOOK_ALIAS ? O_CHN.S_ABOOK_ALIAS : O_CHN.S_SRC
        ].tokens.WNATIVE.symbol.slice(1);
        let s, o;
        for (const O_SRC of oAO_SRC_BSTS[O_CHN.S_SRC]) {
            ...
            if (
                (S_ID.startsWith('bifi-') &&
                    (S_ID.endsWith(
                            '-' + (!O_CHN.S_GVPOOL_SFX_ALIAS ? S_NTV.toLowerCase() : O_CHN.S_GVPOOL_SFX_ALIAS)
                        ) ||
                        S_ID.endsWith(
                            '-' +
                            (!O_CHN.S_GVPOOL_SFX_ALIAS ? S_NTV.toLowerCase() : O_CHN.S_GVPOOL_SFX_ALIAS) +
                    ...
                    'BIFI' === O_SRC.token &&
                    (S_NTV === O_SRC.earnedToken || 'W' + S_NTV === O_SRC.earnedToken)) ||
                'moo_beFTM' === S_ID ||
                'moo_beJOE' === S_ID ||
                'moo_beQI' === S_ID ||
                'moo_beCAKE' === S_ID
            ...
            if ((o_trgt = o_trgtChn[S_ID])) {
                ...
                for (const S_PROP in O_SRC) {
                    ...
                    //	the source's, loop.
                    if (S_PROP in O_PROP_IGNR_ || (!b_drty && o_trgt[S_PROP] === O_SRC[S_PROP])) continue;
                    ...
                    if (mS_PRPNM_PTNRS === S_PROP) {
                        ...
                        O_SRC[S_PROP].forEach((O_PTNR, I) => {
                            ...
                            const O_CTX = O_CMN_PTNR_SRC ? .[mS_PRPNM_CMN_PTNRS].find(
                                O => O_PTNR == O[mS_PRPNM_PTNR]
                            );
                            if (O_CTX) {
                                //if no counterpart common-partner descriptor is present in the target
                                //	environment...
                                const O_ctxTrgt = O_cmnPtnrTrgt[mS_PRPNM_CMN_PTNRS].find(
                                    O => O_CTX[mS_PRPNM_NM] === O[mS_PRPNM_NM]
                                );
                                if (!O_ctxTrgt) {
                                    ...
                                    for (const S in mO_PRPNM_PTNR_GRPHC) {
                                        ...
                                        if (S_FILE === o_trgt[S_PROP][I] ? .[S]) continue;

                                        //copy over the file
                                        try {
                                            copyOverSrcImage(S_FILE);
                                            if (!Array.isArray(O_hit[S_PRPNM_GRPHC]))
                                                O_hit[S_PRPNM_GRPHC] = !O_hit[S_PRPNM_GRPHC] ?
                                                S_FILE : [O_hit[S_PRPNM_GRPHC], S_FILE];
                                            else O_hit[S_PRPNM_GRPHC].push(S_FILE);
                                            console.log(`  Migrated updated graphic of boost ${S_ID}`);
                                        } catch (O) {
                                            console.error(
                                                `  Failed to copy over obstensibly updated graphic af boost ${S_ID}:\n   beefy-app/src/images/${S_FILE} (Error: ${O}`
                                            );
                                        } //try
                                    } //for (const S in mO_PRPNM_PTNR_GRPHC)
                                    ...
                                    if (!O_ctxTrgt[mS_PRPNM_CHKD]) vetCommonPartner(O_PTNR, O_ctxTrgt, S_ID, O_hit);
                                } //if (!O_ctxTrgt)
                                ...
                                if (TrgtCommonPartner.S_DRTY === O_ctxTrgt[mS_PRPNM_CHKD]) b_drty = true;
                                ...
                                for (const S in mO_PRPNM_PTNR_GRPHC) {
                                    ...
                                    try {
                                        ...
                                        console.error(
                                            `  Failed to copy over obstensibly updated graphic of boost ${S_ID}:\n   beefy-app/src/images/${S_FILE} (Error: ${O}`
                                        );
                                    } //try
                                    b_drty = true;
                                } //for (const S in mO_PRPNM_PTNR_GRPHC)
                                ...
                                //	descriptors...
                                const B_TRGT_CMN_PTNR = O_cmnPtnrTrgt[mS_PRPNM_CMN_PTNRS].some(
                                    O => O[mS_PRPNM_PTNR] === o_trgt[S_PROP][I]
                                );
                                if (b_drty || B_TRGT_CMN_PTNR) {
                                    ...
                                    if (!B_TRGT_CMN_PTNR) {
                                        ...
                                        if (o) o_trgt[S_PROP][I][mS_PRPNM_SOCL] = o;
                                    } else o_trgt[S_PROP][I] = O_PTNR;
                                ...
                                for (const S in O_PTNR) {
                                    if (mO_PRPNM_PTNR_GRPHC.hasOwnProperty(S)) continue;
                                    ...
                                    if (b_drty) {
                                        if (mS_PRPNM_SOCL === S) Object.assign(o_trgt[S_PROP][I][S], O_PTNR[S]);
                                        else o_trgt[S_PROP][I][S] = O_PTNR[S];
                                        continue;
                                    } //if (b_drty)

                                    //if the property matches, loop for the next property
                                    if (o_trgt[S_PROP][I][S] === O_PTNR[S]) continue;
                                    ...
                                    //for each social-media sub-property...
                                        ...
                                        for (const S_ in O_PTNR[S]) {
                                            ...
                                            if (!b_drty && o_trgt[S_PROP][I][S][S_] === O_PTNR[S][S_]) continue;
                        ...
                        if (O_SRC[S_PROP] !== o_trgt[S_PROP]) {
                            try {
                                ...
                                console.error(
                                    `  Failed to copy over obstensibly updated logo of boost ${S_ID}:\n   beefy-app/src/images/${O_SRC[S_PROP]} (Error: ${O}`
                                );
                            }
                            b_drty = true;
                        ...
                        if (!b_drty &&
                            o_trgt[S_PROP] &&
                            O_SRC[S_PROP].length == o_trgt[S_PROP].length &&
                            o_trgt[S_PROP].every((S, I) => O_SRC[S_PROP][I] === S)
                        )
                            continue;
                ...
                if (b_drty) O_hit[mS_PRPNM_TYP] = S_TYP_UPDT;
                O_hits[S_PRPNM_HIT] = O_hit;
                if (b_drty) o_trgtChn.b_dirty = true;
            ...
            o = {
                [mS_PRPNM_ID]: S_ID,
                [mS_PRPNM_ID_VLT]: S_VLT_ID,
                ...O_SRC
            };
            if (O_SRC[mS_PRPNM_ASSTS]) o[mS_PRPNM_ASSTS] = [...O_SRC[mS_PRPNM_ASSTS]];
            for (const S in O_PROP_IGNR)
                if (S in o) delete o[S];
            ...
            if (S_LOGO)
                try {
                    ...
                    console.error(
                        `  Failed to copy over specified logo of new boost ${S_ID}:\n   beefy-app/src/images/${S_LOGO} (Error: ${O}`
                    );
                }
            ...
            O_SRC[mS_PRPNM_PTNRS].forEach((O_PTNR, I) => {
                //if this is one of the common partners noted in the chain's source table...
                const O_CTX = O_CMN_PTNR_SRC ? .[mS_PRPNM_CMN_PTNRS].find(O => O_PTNR == O[mS_PRPNM_PTNR]);
                if (O_CTX) {
                    ...
                    if (O_ctxTrgt) {
                        ...
                        //	source's version
                        if (!O_ctxTrgt[mS_PRPNM_CHKD]) vetCommonPartner(O_PTNR, O_ctxTrgt, S_ID, null);
                        ...
                        for (const S in mO_PRPNM_PTNR_GRPHC) {
                            ...
                            try {
                                ...
                                console.error(
                                    `  Failed to copy over graphic noted in boost ${S_ID}:\n   beefy-app/src/images/${O_PTNR[S]} (Error: ${O}`
                                );
                            }
                        } //for (const S in mO_PRPNM_PTNR_GRPHC)
            ...
            o_trgtChn[o[mS_PRPNM_ID]] = o;
            i_added++;
            O_hits[(s = S_TRGT_CHN + ' boost: ' + o[mS_PRPNM_ID])] = {
                [mS_PRPNM_ID]: s,
                [mS_PRPNM_TYP]: S_TYP_ADD,
            };
            o_trgtChn.b_dirty = true;
        ...
        const I = i_pruned;
        Object.values(o_trgtChn).forEach(O_trgt => {
            if (!(O_trgt && Object === O_trgt.constructor)) return;
...
function vetCommonPartner(O_SRC, O_trgt, S_ID, O_hit) {
    ...
    for (const S in mO_PRPNM_PTNR_GRPHC) {
        ...
        if (S_FILE === O_trgt[mS_PRPNM_PTNR][S]) continue;
        ...
        try {
            ...
            if (O_hit)
                if (!Array.isArray(O_hit[S_PRPNM_GRPHC]))
                    O_hit[S_PRPNM_GRPHC] = !O_hit[S_PRPNM_GRPHC] ? S_FILE : [O_hit[S_PRPNM_GRPHC], S_FILE];
                else O_hit[S_PRPNM_GRPHC].push(S_FILE);
            console.log(`  Migrated updated graphic of boost ${S_ID}`);
        } catch (O) {
            console.error(
    ...
    if (O_trgt[mS_PRPNM_CHKD]) {
        ...
        if (O_socl) Object.assign(O_socl, O_SRC[mS_PRPNM_SOCL]);
        Object.assign(O_trgt[mS_PRPNM_PTNR], O_SRC);
        if (O_socl) O_trgt[mS_PRPNM_PTNR][mS_PRPNM_SOCL] = O_socl;
        //else mark the target's common-partner descriptor as now vetted
    } else O_trgt[mS_PRPNM_CHKD] = TrgtCommonPartner.S_VETD;
} //vetCommonPartner(
...
async function P_main() {
    ...
    try {
        ...
        Object.values(o_HITS).forEach(O => {
            if (!O[mS_PRPNM_TYP]) delete o_HITS[O[mS_PRPNM_ID]];
        });
        mO_FS.writeFileSync(
            `${mS_DIR_BASE}/beefy-v2/V1objectsMigrated.log`,
            JSON.stringify(Object.values(o_HITS), null, 2)
        );
    } catch (E) {
        console.error(E);
    }
